...
function scene_push(_name, _payload) {
    ...
    var cb = __scene_registry[? _name]; array_push(__scene_stack, [_name, cb]);
    if (is_struct(cb) && is_method(cb.on_enter)) cb.on_enter(_payload);
}
function scene_pop() { if (array_length(__scene_stack) > 0) { var t=array_pop(__scene_stack); var cb=t[1]; if (is_struct(cb)&&is_method(cb.on_exit)) cb.on_exit(); } }
...
function scene_update(){ var t=scene_top(); if(!is_array(t))return; var cb=t[1]; if(is_struct(cb)&&is_method(cb.on_update))cb.on_update(); if(is_struct(cb)&&is_method(cb.on_input))cb.on_input(); }
function scene_draw(){ var t=scene_top(); if(!is_array(t))return; var cb=t[1]; if(is_struct(cb)&&is_method(cb.on_draw))cb.on_draw(); }
